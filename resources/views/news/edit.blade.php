...
@section('content')
    <main class="d-flex bg-darkgray">
        ...
        <section id="createForm" class="d-flex flex-column col-10 justify-content-center mx-auto py-5">
            <form class="d-flex flex-column text-light gap-4" action="{{ url('/admin/noticias/editar/' . $new->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <h2 class="fs-4">Noticias</h2>
                <div class="d-flex flex-column gap-4">
                    <div class="d-flex flex-column">
                        <textarea class="fs-3 bg-transparent p-1 text-blueultra" id="title" name="title" placeholder="Título de la noticia">{{ old('title', $new->title) }}</textarea>
                        @error('title')
                ...
                <button class="hvr-shutter-out-horizontal fs-5 my-3 text-light rounded-pill py-1 px-3 w-50 mx-auto my-4 border-blueultra" type="submit">Publicar</button>
            </form>
        </section>
    </main>
@endsection
